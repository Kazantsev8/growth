...
### Структурные токены (Фаза 2, в `tokens.css`)

- `--bw: 1.5px` — **толщина рамок**, основной примитив. **Адаптирован повсеместно**: все `border`/`outline`
  в `app/css/**` используют `var(--bw)` (замена значение-в-значению — вид не изменился).
- `--r: 2px` — малый радиус (ячейки/столбики/чипы). Определён; применять в **новых** компонентах.
- `--sp-1..--sp-5` (6/10/14/18/22px) — **рекомендованная шкала отступов для НОВЫХ компонентов**.
  Существующие значения исторически ad-hoc и намеренно НЕ переписывались (churn без пользы + риск
  визуальных регрессий при «UX без изменений»).
