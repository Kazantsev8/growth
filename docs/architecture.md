...
- **Фаза 0 — каркас документации** ✅ (этот документ и система доков созданы).
- **Фаза 1 — модульная экстракция** ⏳ (код пока монолитный в `app/index.html`, ~1873 строки).
- **Фаза 2 — дизайн-система** ⏳.
- **Фаза 3 — перформанс (ленивые импорты, SW)** ⏳.
- **Фаза 4 — финал доков** ⏳.

Этот раздел обновляется по мере прохождения фаз (см. протокол в [Claude.md](Claude.md)).
До завершения Фазы 1 фактическая раскладка — единый `index.html`; ниже описана **целевая** структура.
...
Вендоры (`js-yaml`, `marked`) подключены классическими `<script>` в `<head>` и доступны
как глобалы `window.jsyaml` / `window.marked`; доступ к ним централизован в `core/vendor.js`.
...
js/
  ...
  kinds/
    registry.js         kind → ленивый загрузчик import() + meta
